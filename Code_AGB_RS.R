...
#### SINGLE VARIABLE CONDITIONAL MODEL RESPONSE
...
#### rename Random Forest coloumn names
colnames(DATA_BIOMASS)[25] <- 'Shadow_Index'
colnames(DATA_BIOMASS)[11] <- 'R1SD1'
colnames(DATA_BIOMASS)[12] <- 'R2SD2'
colnames(DATA_BIOMASS)[13] <- 'R3SD3'
colnames(DATA_BIOMASS)[14] <- 'R4SD4'
colnames(DATA_BIOMASS)[15] <- 'R5SD5'


## re create a RF model
rf_H_with <- randomForest(BIOMASS ~., data = DATA_BIOMASS,importance = TRUE, proximity = T,ntree=500)

## create an explainer
explainer <-  explain(rf_H_with, data = DATA_BIOMASS)


### plot single respomnse var
expl_rf <- single_variable(explainer, variable = 'Shadow_Index', type ='pdp' )
plot(expl_rf)+theme_minimal()+ylab('AGB')+xlab('Shadow Index')+ theme(legend.position="none")
expl_rf1 <- single_variable(explainer, variable = 'R2SD2', type ='pdp' )
plot(expl_rf1)+theme_minimal()+ylab('AGB')+xlab('R2SD2')+ theme(legend.position="none")
